...
function Trigger( a, b ){
	...
	if( a instanceof Instrument ){

		this.instrument = a
		if( b instanceof Note ) this.note = b
		else if( b !== undefined ) this.note = new Note( b )
	}
	else if( a instanceof Note ){

		this.note = a
		if( b instanceof Instrument ) this.instrument = b
	}
	else if( a !== undefined ){

		this.note = new Note( a )
	}
	...
	else this.context = window.AudioContext ? new AudioContext() : new webkitAudioContext()
	...
	this.domContainer.addEventListener( 'touchstart', function( event ){
		...
		that.engage( 'touchstart' )
		event.preventDefault()
	})
	this.domContainer.addEventListener( 'touchend', function( event ){

		that.disengage( 'touchstart' )
		event.preventDefault()
	})
}


Trigger.prototype.addEventListener = function( type, action ){
...
Trigger.prototype.removeEventListener = function( type, action ){
...
Trigger.prototype.addTriggerChar = function( trigger ){
...
Trigger.prototype.createVoices = function(){
	...
	note0  = this.note
	voice0 = new Voice( note0, this.context )
	...
	note1  = new Note( note0.hertz / 2 )
	voice1 = new Voice( note1, this.context )
...
Trigger.prototype.destroyVoices = function(){
...
Trigger.prototype.engage = function( eventType ){
...
Trigger.prototype.disengage = function( eventType ){
